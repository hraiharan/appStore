...
class AppStore extends Component {
  state = {todoId: 'SOCIAL', searchInput: ''}
  changeCatogory = itemId => {
    this.setState({todoId: itemId})
  }
  onChangeIput = event => {
  ...
  render() {
    const {todoId, searchInput} = this.state
    const finalAppList = appsList.filter(eachItem => {
      return (
        eachItem.category === todoId &&
    ...
    return (
      <div className="main-container">
        <div className="sub-container">
          ...
          <ul className="tab-item-conatiner">
            {tabsList.map(eachItem => (
              <TabItem
                active={eachItem.tabId === todoId}
                onclick={this.changeCatogory}
                props={eachItem}
                key={eachItem.tabId}
              />
            ))}
          </ul>
          <ul className="app-item-conatiner">
            {finalAppList.map(eachItem => (
              <AppItem props={eachItem} key={eachItem.appId} />
            ))}
          </ul>
        </div>
      </div>
